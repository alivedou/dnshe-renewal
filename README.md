...
3. 全部跑完后 **一条** Telegram 精简汇总（每账号一行统计）  
4. 详细逐域名日志只在 Actions 控制台，不推 TG  
5. 任一账号有续期/列表失败时，Workflow 以 exit code 1 结束（方便 Actions 标红），但前面账号已执行完  
...
【号1】共 8 个域名，无需续期 6，续期成功 1，续期失败 1
失败域名:
